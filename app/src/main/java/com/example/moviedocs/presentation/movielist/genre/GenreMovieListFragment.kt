...
@AndroidEntryPoint
class GenreMovieListFragment : BaseFragment<FragmentMovieListBinding>(
  FragmentMovieListBinding::inflate
) {

  private val args: GenreMovieListFragmentArgs by navArgs()

  private val viewModel: GenreMovieListViewModel by viewModels()
  ...
  private fun setUpNavigation() {
    ...
    movieListAdapter.setOnItemClickListener { it: MovieItemModel ->
      findNavController().navigate(
        GenreMovieListFragmentDirections.actionGenreMovieListFragmentToMovieDetailFragment(
          movieId = it.id
        )
      )
    }
  ...
  private fun handleLoadNextPage() {
    pageNumbersAdapter.setOnItemClickListener { it: Int ->
      viewModel.loadPage(it, args.genreId)
    }
  }
