...
@AndroidEntryPoint
class SimilarMovieListFragment : BaseFragment<FragmentMovieListBinding>(
  FragmentMovieListBinding::inflate
) {

  private val args: SimilarMovieListFragmentArgs by navArgs()

  private val viewModel: SimilarMovieListViewModel by viewModels()
  ...
  override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
    ...
    binding.movieListTitle.text = "Similar"
    setUpNavigation()
    setUpRecyclerView(
  ...
  private fun renderUi(state: MovieListUiState) {
    when (state) {
      ...
      is MovieListUiState.Error -> {
        ...
        showErrorDialog { viewModel.loadPage(args.movieId, 1) }
      }
    }
  }
  ...
  private fun handleLoadNextPage() {
    pageNumbersAdapter.setOnItemInAdapterClickListener { it: Int ->
      viewModel.loadPage(args.movieId, it)
    }
  }
